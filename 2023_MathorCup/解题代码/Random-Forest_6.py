from MMtools import get_data_dict_6
...
time_series_1_dict=get_data_dict()#附件1的时间序列字典
time_series_6_dict=get_data_dict_6()#附件6的时间序列字典

#删除附件一在双十一未出现的维度
keys_to_delete = []
for key in time_series_1_dict.keys():
    if key not in time_series_6_dict:
        keys_to_delete.append(key)

for key in keys_to_delete:
    del time_series_1_dict[key]

#time_series_1_dict为长1957的字典 key与time_series_6_dict完全一样 但顺序不同

#重排字典
new_time_series_1_dict = OrderedDict((key,time_series_1_dict[key]) for key in time_series_6_dict)

#new_time_series_1_dict和time_series_6_dict都是长为1957的字典 其key顺序完全一样

time_series_1_lists=list(new_time_series_1_dict.values())#1957个长为166的时间序列
time_series_6_lists=list(time_series_6_dict.values())#1957个长短不一的时间序列

#叠加电商波动相对指数的均值回归

output_folder = 'E:/plot_6'
...
for time_series in time_series_1_lists:
    # 创建一个模拟的时间序列列表
    ...
    #计算R
    g,f=cluR(time_series,time_series_6_lists[d])
    R=0.5*(2*g+f)
    print(f'{R}')
    R_6.append(R)
    d+=1
    ...
    max_values = sorted(forecast_qty)[-5:]

    # 创建一个新的列表，对于每个元素，如果它是最大的五个元素之一，就乘以(1 + R)，否则保持不变
    new_list = [(1 + R) * element if element in max_values else element for element in forecast_qty]

    new_list[17]=new_list[17]*1.5

    forecast_qty=new_list

    time_series_keys=get_keys_by_value(time_series_1_dict, time_series)#取得时间序列对应的xyz元组 并保存在列表中
    ...
    # 第三个子图：未来十五步的预测结果
    plt.subplot(3, 1, 3)
    plt.plot(range(len(time_series)), time_series, label='Known Time Series', color='blue')
    plt.plot(range(len(time_series), len(time_series) + 15), forecast[:15], label='Next 15 Steps Predictions', linestyle='--', color='blue')
    plt.plot(range(len(time_series) + 15, len(time_series) + 35), forecast_qty, label='Following 20 Steps Predictions', linestyle='--', color='red')
    plt.title('Predicted Results For The Next 20 Steps', fontsize=11)
    plt.xlabel('Time Step', fontsize=11)
    plt.ylabel('Value', fontsize=11)
    plt.legend()
...
# 保存工作簿为 Excel 文件
workbook.save('E:/Data_storage/result6.xlsx')
