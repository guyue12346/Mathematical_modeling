...
def get_LSTM_Data(k):
    list_xyz=get_xyz_tpl(k)#list_xyz包含了聚类分为十类的xyz列表组合
...
def deal_X(data):
    ...
    for time_series in data:
        # 提取前165个时间步作为输入
        input_sequence = time_series[:165]
        # 提取后165个时间步作为目标变量
        target_sequence = time_series[165:]
...
def calculate_wmape(y_test, y_pred, weights=None):
    """
    计算加权平均绝对百分比误差（WMAPE）。

    参数：
    y_test (list): 实际目标值的列表。
    y_pred (list): 模型预测的值的列表。
    weights (list, optional): 可选的权重列表。如果未提供，将假定所有权重均为1。

    返回：
    wmape (float): 加权平均绝对百分比误差。
    """
    if weights is None:
        weights = [1] * len(y_test)
    
    assert len(y_test) == len(y_pred) == len(weights), "Input lists must have the same length."

    wmape_sum = 0
    total_weight = 0

    for actual, predicted, weight in zip(y_test, y_pred, weights):
        wmape_sum += weight * (abs(actual - predicted) / (abs(actual) + 1e-8))
        total_weight += weight

    wmape = (wmape_sum / total_weight) * 100.0

    return 1 - (wmape / 100.0)

def calculate_1_wmape(y_test, y_pred):
    """
    计算平均绝对百分比误差（MAPE）。
    ...
    返回：
    mape (float): 平均绝对百分比误差（MAPE）。
    """

    assert len(y_test) == len(y_pred), "Input lists must have the same length."

    mape_sum = 0

    for actual, predicted in zip(y_test, y_pred):
        mape_sum += abs(actual - predicted) / (abs(actual) + 1e-8)

    mape = (mape_sum / len(y_test)) * 100.0
